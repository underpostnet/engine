...
const runQuiet = ({ tty = false, plain = false, exitCode = 0 } = {}) => {
...
describe('deploy log table (run_quiet filter)', () => {
  ...
  it('emits one plain row per pod state change, iteration first and the iteration clock second', () => {
    const rows = runQuiet()
      .split('\n')
  ...
  it('heads the plain rows once and keeps non-monitor lines', () => {
    const out = runQuiet();
    const headers = out.split('\n').filter((line) => line.includes('ITERATION'));

    expect(headers).to.have.lengthOf(1);
    expect(headers[0]).to.equal('| ITERATION | TIMESTAMP | POD NAME | K8S STATUS | RUNTIME STATUS |');
  ...
  it('falls back to plain rows on a terminal when RUN_QUIET_PLAIN is set', function () {
    ...
    expect(out).to.include(`| #3 | 08:11:33 | ${POD_A} | Running (ready) | running-deployment |`);
    expect(out).to.not.include('deploy-monitor');
  });

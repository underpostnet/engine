/**
 * Shared command surface for the three configuration domains.
 *
 * `secret`, `host` and `app` are registered from one factory, so the action list and the option
 * list exist exactly once. Symmetry is not a convention the three commands agree to follow — it
 * is structural: a verb or a flag added here appears on all three, and one that is not here
 ...
 * @param {string} domain.name - Command name (`secret`, `host`, `app`).
 ...
 * @memberof UnderpostDomains
 */
const registerDomainCommand = (program, { name, description, api }) => {
  const command = program
    .command(name)
    .argument(
      '<action>',
      `Action to run. One of: ${DOMAIN_ACTIONS.map((action) => action.name).join(', ')}.\n` +
        DOMAIN_ACTIONS.map((action) => `  ${action.name.padEnd(8)} ${action.summary}`).join('\n'),
    )
    .description(description)
    .action((action, options) => {
      const known = DOMAIN_ACTIONS.map((entry) => entry.name);
      if (!known.includes(action)) throw new Error(`Unknown ${name} action: ${action} (expected ${known.join(', ')})`);
...
export { DOMAIN_ACTIONS, DOMAIN_OPTIONS, domainContextFactory, parseDomainArgs, registerDomainCommand };

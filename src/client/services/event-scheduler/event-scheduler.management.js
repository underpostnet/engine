...
const EventSchedulerOptions = {
  idModal: 'modal-event-scheduler-management',
  serviceId: 'event-scheduler-management',
  ...
  event-schedulerColKeyFocus: '0',
  ServiceProvider: EventSchedulerService,
  permissions: {
    add: true,
    remove: true,
  },
...
const EventSchedulerManagement = {
  Tokens: {},
  RenderTable: async function (options = EventSchedulerOptions) {
    if (!options) options = EventSchedulerOptions;
    const { serviceId, columnDefs, entity, event-schedulerColKeyFocus, ServiceProvider, permissions } = options;
    logger.info('EventSchedulerManagement RenderTable', options);
      ...
      EventsUI.onClick(
        ...
        async () => {
          ...
          if (result.status === 'success') {
            ...
            //   ],
            //   event-schedulerState: { sort: null },
            // });
          } else {
            // AgGrid.grids[gridId].applyColumnState({
            //   event-schedulerState: { sort: null },
          ...
          setTimeout(() => {
            AgGrid.grids[gridId].startEditingCell({
              rowIndex: 0,
              colKey: event-schedulerColKeyFocus,
              rowPinned: pinned,
              key: key,
            });
          });

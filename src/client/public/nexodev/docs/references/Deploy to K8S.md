...
SSR views whose route is a bare status code (`/404`) and views flagged `offlineDefault` / `maintenanceDefault` are served by the `gateway-static-utility` Nginx workload rather than by the application pods — a rendered page is far past the 4096-byte direct-response ceiling the gateway would otherwise have to carry, and exceeding it fails the whole route.

...
this.cyberiaonline0 = {
    ...
    init: function () {
        ...
        const renderPosition = (cord, element) =>
            element[cord] * (cyberiaonline0.canvasDim / maxRangeMap) - (element.dim * pixiAmplitudeFactor);
        ...
        const PIXI_INIT = () => {
            ...
            // /assets/apps/cyberiaonline0/clases
        ...
        const COMPONENTS = {
            'coin-indicator': {
                ...
                event: function (element, value) {
                    ...
                    append(cyberiaonline0.htmlPixiFontLayer, /*html*/`
                            <span class='abs ${fontEffectId}' style='
                            top: ${((element.y * cyberiaonline0.canvasDim) / maxRangeMap) * randomIndicatorPosition()}px;
                            left: ${((element.x * cyberiaonline0.canvasDim) / maxRangeMap) * randomIndicatorPosition()}px;
                            color: yellow;
                            font-family: retro;
                    ...
                    switch (this.type) {
                        case 'USER_MAIN':
                            ...
                            this.onCanvasClick = event => {
                                ...
                                // off -> this.canvasDim
                                // x -> 50
                                let offsetX = parseInt(((event.offsetX * maxRangeMap) / cyberiaonline0.canvasDim)) + 1;
                                let offsetY = parseInt(((event.offsetY * maxRangeMap) / cyberiaonline0.canvasDim)) + 1;
    ...
    offFullScreen: function () {
        console.warn('pixijs cyberiaonline0 | offFullScreen');
        s(`.${this.inFullScreenBtn}`).style.display = null;
        s(`.${this.outFullScreenBtn}`).style.display = 'none';
    },
    onFullScreen: function () {
        console.warn('pixijs cyberiaonline0 | onFullScreen');
    ...
    changeWindowDimension: function (dimensionData) {
        console.log('pixijs cyberiaonline0 | changeWindowDimension', dimensionData);

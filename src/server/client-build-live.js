  ...
  if (fs.existsSync(`./tmp/client.build.json`)) {
    ...
    for (let srcPath of updates) {
      if (
        ...
        const buildPath = `./public/${baseHost}/${srcPath.split('src')[1].slice(8)}`.replace(/\\/g, '/');
        liveClientBuildPaths.push(buildPath);
      } else if (srcPath.split('src')[1].startsWith(`\\client\\sw`)) {
        const buildPath = `./public/${baseHost}/sw.js`;
        liveClientBuildPaths.push(buildPath);
        ...
        for (const view of Config.default.client[clientId].views) {
          const buildPath = `./public/${baseHost}${view.path === '/' ? '' : view.path}/${clientId}.index.js`;
          liveClientBuildPaths.push(buildPath);
        }
      } else if (srcPath.split('src')[1].startsWith(`\\client\\ssr`)) {
        for (const view of Config.default.client[clientId].views) {
          const buildPath = `./public/${baseHost}${view.path === '/' ? '' : view.path}/index.html`;
          liveClientBuildPaths.push(buildPath);
        }
      }
    }

...
const repoPull = (repoPath = './', repoUri = 'underpostnet/pwa-microservices-template') => {
  shellExec(`cd ${repoPath} && git pull https://${process.env.GITHUB_TOKEN}@github.com/${repoUri}.git`, {
    disableLog: true,
  });
};
...
const repoPush = (repoPath = './', repoUri = 'underpostnet/pwa-microservices-template') => {
  shellExec(`cd ${repoPath} && git push https://${process.env.GITHUB_TOKEN}@github.com/${repoUri}.git`, {
    disableLog: true,
  });
  logger.info(
    'commit url',

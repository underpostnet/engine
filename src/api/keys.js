...
const blockChainConfig = JSON.parse(fs.readFileSync(
    `./underpost_modules/underpost-data-template/network/blockchain-config${process.env.NODE_ENV == 'development' || process.env.NODE_ENV == 'test-dev' || process.env.NODE_ENV == 'ipfs-dev' || process.env.NODE_ENV == 'cyberia-dev' ? '.dev' : ''}.json`,
    'utf8'
));
